...
namespace cev_planner::global_planner {
    ...
    public:
        ...
        Trajectory plan_path(Grid grid, State start, State target) override {
            ...
            mapw = grid.data.rows();
            maph = grid.data.cols();
            nodes = {{0, {max(min((int)round((start.pose.x - grid.origin.x) / resolution), mapw - 1), 0), max(min((int)round((start.pose.y - grid.origin.y) / resolution), maph - 1), 0)}}};
            ...
            while (true) {
                ...
                if (iteration % 500 == 0 and goalFlag and 
                    (abs(bestCost - distToGoal) < 1 or 
                    (in_region / log(bestCost+2) > min(max(bestCost/2000*75+275, 275.0), 350.0))))
                    break;

                expand();
                if (!goalFlag and iteration % 500 == 0)
                    updateKDTree();
        ...
        class Coordinate {
            ...
            Pose as_is_pose() const { return {static_cast<double>(x), static_cast<double>(y)}; }
        };
        ...
        class Node {
            ...
            double cost;
            Node(int x=0, int y=0, double cost=0) : coordinate(x, y), x(x), y(y), cost(cost), parent(-1) {}
            Node(Coordinate& coordinate, double cost=0) : coordinate(coordinate), x(coordinate.x), y(coordinate.y), cost(cost), parent(-1) {}
    
            void set_parent(int parent) { this->parent = parent; }
            void set_cost(double cost) { this->cost = cost; }
            void add_child(int child) { this->children.insert(child); }
            void remove_child(int child) { this->children.erase(child); }
            double distance_to(const Node& other) const { return coordinate.distance_to(other.coordinate); }
            Pose as_is_pose() const { return coordinate.as_is_pose(); }
            Pose tf_pose(double resolution, Pose& origin) const { return coordinate.tf_pose(resolution, origin); }
        };        
    private:
    ...
    int mapw, maph, iter, num_not_in_region, num_max_in_region,
        cur_index, cur_goal_index, best;
    ...
    void dilate_grid(
        ...
        const Node& start,
        const Node& goal,
        const std::vector<array<int, 2>>& structure = {{0,1}, {1,0}, {0,0}, {-1,0}, {0,-1}},
        double safe_zone_radius = 5.0,
        float occupation_threshold = 0.7f
    );
    ...
    int merge_trees(int node, int cur_other, KDTree& kdTree, unordered_map<int, Node>& nodes, unordered_map<int, Node>& other_nodes);
    ...
    bool is_ancestor(int potential_ancestor, int node, unordered_map<int, Node>* nodes = nullptr);
    ...
    double estimatedCost(const Coordinate& coordinate) { return coordinate.distance_to(start.coordinate) + coordinate.distance_to(goal.coordinate); }
    ...
    void rewire_edge(int parent, int old_child, int new_child, unordered_map<int, Node>* nodes = nullptr) {
        nodes = nodes ?: cur_tree;
